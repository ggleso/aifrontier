...
<body>
  ...
  <main class="wrap">
    ...
    <div class="workspace">
      ...
      </div><div class="input-box"><label for="utterance"><b>고객 발화 입력</b></label><textarea id="utterance" maxlength="2000" placeholder="합성 상담 문장을 입력하세요"></textarea><div class="input-actions"><button class="ghost" id="mic" type="button">🎙 음성 인식</button><button class="ghost" id="fake-stt" type="button">Fake STT</button><button class="primary" id="analyze" type="button">분석</button></div></div></section>
      <div class="center">
  ...
  <script>
    ...
    const analyze = async () => {
      ...
      if (!text) { status.textContent = '분석할 합성 발화를 입력하세요.'; status.className = 'listening error'; return; }
      appendMessage('고객', text); input.value = ''; eventNumber += 1;
      document.querySelector('#event').textContent = '발화 #' + String(eventNumber).padStart(2, '0');
      status.textContent = '로컬 Copilot이 분석 중입니다…'; status.className = 'listening';
      ...
      try {
        ...
        status.textContent = '분석 완료 · 다음 발화를 기다립니다.'; status.className = 'listening success';
      } catch { status.textContent = '분석에 실패했습니다. 서버 상태를 확인하세요.'; status.className = 'listening error'; }
    };
    document.querySelector('#analyze').addEventListener('click', analyze);
    document.querySelector('#fake-stt').addEventListener('click', () => { input.value = '다음 주 급여일 전까지 카드대금과 통신비 납부가 걱정돼요.'; document.querySelector('#input-provider').textContent = 'Fake STT'; status.textContent = 'Fake STT 합성 발화를 불러왔습니다.'; });
    document.querySelector('#mic').addEventListener('click', () => {
      ...
      recognition.onerror = () => { status.textContent = '음성 인식 실패 · Fake STT를 이용할 수 있습니다.'; status.className = 'listening error'; };
      recognition.start();
    });
